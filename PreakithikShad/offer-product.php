...
?>
    ...
    <section class="combo-section">
        <div class="container">
            <h2 class="combo-section-title">অফার প্রোডাক্ট</h2>
            <div class="row">
				<?php 
					$select = "SELECT * FROM products WHERE categories_id = 12"; 
					$stmt = $pdo->prepare($select);
					$stmt->execute();
					$offerProducts = $stmt->fetchAll();
					
					if(empty($offerProducts)){
						echo "<h2 style='color:red;'>কোন অফার পণ্য খুজে পাওয়া জাইনি</h2>";
					}
					
					foreach($offerProducts as $offerProduct){
						$oldPrice = $offerProduct['old_price'];
						$newPrice = $offerProduct['product_price'];
						$percentLess = abs((($oldPrice - $newPrice) / $oldPrice) * 100);
						
						$description = $offerProduct['product_dsc'];
						...
						?>
							<div class="col-lg-3 col-md-4 col-6 mb-4">
								<div class="combo-card card h-100" onclick="window.location.href='article-details.php?blog=<?php echo $offerProduct['product_name']; ?>'">
									<div class="position-relative">
										<img src="assets/<?php echo $offerProduct['image']; ?>" class="combo-img card-img-top" alt="<?php echo $offerProduct['product_name']; ?>">
										<div class="combo-badge"><?php echo $percentLess; ?>% ছাড়</div>
									</div>
									<div class="card-body">
										<h5 class="combo-title">
											<?php echo $offerProduct['product_name']; ?>
										</h5>
										<p class="combo-products"><?php echo $description; ?> </p>
										<div class="d-flex align-items-center">
											<span class="combo-price"><?php echo $newPrice; ?> টাকা</span>
											<span class="combo-old-price"><?php echo $oldPrice; ?> টাকা</span>
										</div>
										<a href="add_to_cart.php?id=<?= $offerProduct['product_id'] ?>" class="cart-product-link">
