    ...
    <div class="main-content">
		...
		<?php 
			if(isset($_POST['RechargeBtn'])){
				$coin_amount = isset($_POST['recharge_amount']) ? (float) $_POST['recharge_amount'] : 0;
                $convert_amount = $coin_amount / 2;
				
				if (!is_numeric($coin_amount) || $coin_amount <= 0){
					...
					if(is_numeric($coin_amount) && isset($coin) && $coin >= $coin_amount){
						if($convert_amount >= 100 && $convert_amount <= 500){
							if($alreadyReachargeToday < 1){
								$sql = "INSERT INTO transfer(user_id, transfer_amount) VALUES (?, ?)";
