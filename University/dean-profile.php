<?php
	...
	if($_GET['profile']){
		$name = $_GET['profile'];		
		$sql = "SELECT * FROM dean WHERE dean_name=?";
		$stmt = $connection->prepare($sql);
		$stmt->execute([$name]);
		$row = $stmt->fetch();
		
		$id = $row['dean_id'];
		$sql2 = "SELECT * FROM dean_education WHERE dean_id=?";
		$stmt2 = $connection->prepare($sql2);
		$stmt2->execute([$id]);
		
		$sql3 = "SELECT * FROM dean_work WHERE dean_id=?";
	...
	<main class="profile-main">
		<section class="profile-section basic-info">
			...
			<div class="info-wrapper">
				<img src="assets/<?php echo $row['dean_image']; ?>" alt="Administrator Photo" class="admin-photo">
				<ul>
					<li> <strong> name </strong> <?php echo $row['dean_name']; ?> </li>
				</ul>
			</div>
		</section>
